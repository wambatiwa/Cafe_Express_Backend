...
router.post('/getPdf',auth.authentificationToken,function(req,res) {
    const orderDetails = req.body;
...
router.get("/getBills", auth.authentificationToken, (req, res, next) => {
  var query = "select * from bill order by id DESC";
  connection.query(query, (err, results) => {
    if (!err) {
      if (results.length == 0)
        return res.status(200).json({ message: "No bills yet available" });
  ...
  connection.query(query,[id],(err,results) => {
    if(!err) {
      if(results.affectedRows == 0) {
        return res.status(404).json({message: "This bill does not exist"});
      }
      return res.status(400).json({message: "Bill deleted successfully"});
    }
    else {
      return res.status(500).json(err);
    }
